...
class LearnerTest {
    ...
    void testCanBookLesson_SameGrade() {
        assertTrue(learner.canBookLesson(learner.getGradeLevel()), "Learner should be able to book a lesson of the same grade level.");
    }

    @Test
    void testCanBookLesson_OneLevelHigher() {
        assertTrue(learner.canBookLesson(learner.getGradeLevel() + 1), "Learner should be able to book a lesson one grade level higher.");
    }

    @Test
    void testCanBookLesson_TwoLevelsHigher() {
    ...
    @Test
    void testCanBookLesson_LowerGrade() {
        assertFalse(learner.canBookLesson(learner.getGradeLevel() - 1), "Learner should not be able to book a lesson of a lower grade level.");
    }

    @Test
    void testUpdateGradeLevel_SameGrade() {
        int originalGrade = learner.getGradeLevel();
        learner.updateGradeLevel(originalGrade);
        assertEquals(originalGrade, learner.getGradeLevel(), "Grade level should remain unchanged when updated to the same value.");
    }

    @Test
    void testUpdateGradeLevel_IncreaseGrade() {
        int newGrade = learner.getGradeLevel() + 1;
        learner.updateGradeLevel(newGrade);
        assertEquals(newGrade, learner.getGradeLevel(), "Grade level should be updated to the new value.");
    }
